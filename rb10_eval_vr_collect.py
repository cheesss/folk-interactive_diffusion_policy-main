...
def main(
    ...
    servo_proc = None
    servo_log_path = "/tmp/servo_vr_ros2.log"

    use_launcher = os.environ.get("RB10_LAUNCHER", "0") == "1"
    ...
    def _start_servo_vr():
        ...
        cmd = [
            "python3",
            "/home/son_rb/rb_ws/src/robotory_rb10_ros2/scripts/servo_vr_ros2.py",
            "--robot_ip",
            robot_ip,
            "--target_frame",
            target_frame,
        ...
        try:
            log_fp = open(servo_log_path, "ab")
        except Exception:
            log_fp = subprocess.DEVNULL
        servo_proc = subprocess.Popen(cmd, stdout=log_fp, stderr=subprocess.STDOUT)
        print(f"[TELEOP] started servo_vr_ros2 (pid={servo_proc.pid})", flush=True)
    ...
    def _pub_teleop_control(value: int):
        cmd = [
            "ros2", "topic", "pub", "--once",
            "/teleop_control", "std_msgs/msg/UInt8",
            f"{{data: {value}}}"
        ]
        try:
    ...
    try:
        ...
        with SharedMemoryManager() as shm_manager:
            ...
            print("[DEBUG] before RealEnv init", flush=True)
            with RealEnv(
                output_dir=output,
                robot_ip=robot_ip,
                frequency=frequency,
                ...
                print("[DEBUG] after RealEnv init", flush=True)
                print("[DEBUG] before VR bridge start", flush=True)
                vr_node, vr_exec, vr_thread = _start_vr_bridge(target_frame)
                print("[DEBUG] after VR bridge start", flush=True)
                cv2.setNumThreads(1)
                key_reader.start()
                print("Waiting for realsense...")
                if hasattr(env, "is_ready"):
                    print(f"[READY] waiting for env readiness... (initial is_ready={env.is_ready})", flush=True)
                    ready_t0 = time.monotonic()
                    last_ready_log = 0.0
                    while not env.is_ready:
                        if time.monotonic() - last_ready_log > 1.0:
                            rs_ready = getattr(env, "realsense", None)
                            rb_ready = getattr(env, "robot", None)
                            rs_flag = rs_ready.is_ready if rs_ready is not None else None
                            rb_flag = rb_ready.is_ready if rb_ready is not None else None
                            print(f"[READY] realsense={rs_flag} robot={rb_flag}", flush=True)
                            last_ready_log = time.monotonic()
                        if time.monotonic() - ready_t0 > 15.0:
                            raise RuntimeError("RealEnv not ready after 15s. Check robot/camera processes.")
                        time.sleep(0.1)
                time.sleep(1.0)
                ...
                # Stop periodic dump once initialization is complete.
                faulthandler.cancel_dump_traceback_later()

                while True:
                    _wait_for_key("s", key_reader)
                    print("Controls: t=teleop, p=policy, o=open, c=close, q=quit", flush=True)
                    ...
                    while True:
                        ...
                        if key == "t":
                            if mode != "teleop":
                                _start_servo_vr()
                                _pub_teleop_control(1)
                                if hasattr(env, "reset_interpolator"):
                                    env.reset_interpolator()
                        ...
                        if mode == "policy":
                            ...
                            # teleop mode: skip visualization + run faster loop
                            obs = env.get_obs()
                            # teleop mode: label as 1
                            _append_demo_obs(demo_buffer, obs, 1)
                            # faster loop for teleop responsiveness
                            precise_wait(0.01)
                    ...
                    # ensure robot stops receiving teleop commands before finalize/save
                    _pub_teleop_control(0)
                    time.sleep(0.3)
